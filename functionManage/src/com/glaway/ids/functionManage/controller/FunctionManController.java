...
public class FunctionManController {
    ...
    public void addUser(@RequestBody String userDataJson, HttpServletResponse response) {
        ...
        try {
            ...
            // *PERSON <user_id>;<org_id>;<FirstName>;<LastName>;<Phone>;<Address>;<Email>;$
            //为空则用$代替
            List<Map<String, String>> hrUserList = (List<Map<String, String>>) userDataMap.get("data");
            for (Map<String, String> hrUserMap : hrUserList) {
                String userId = hrUserMap.get("userCode");
                String userName = hrUserMap.get("userName");
                //人员状态
                String status = hrUserMap.get("status");
                String gender = hrUserMap.get("gender");
                //人员密级
                String secret = hrUserMap.get("secret");
                //业务组织编码
                String orgId = hrUserMap.get("ywbm");
            ...
            String errorMsg = EncodeUtil.getEncoding(e.getMessage());
            setResult(resultMap, "500", errorMsg);
            response.setContentType("text/html;charset=utf-8");
            ResponseJsonUtils.json(response, resultMap);
            return;
        }
        setResult(resultMap, "200", "success");
        response.setContentType("text/html;charset=utf-8");
        ResponseJsonUtils.json(response, resultMap);
    }
...
//            writeText(fileName, content, "import");
    ...
    private void setResult(Map<String, String> resultMap, String code, String msgValue) {
        resultMap.put("code", code);
        resultMap.put("message", msgValue);
    }
}
